...
export { ElevateElement };
    ...
    constructor(elementName) {
      super();
      console.log(`[BaseElement] Constructing ${elementName || 'unnamed element'}`);
      this.elementName = elementName || this.tagName.toLowerCase();
      this.initialized = false;
    }
    ...
    async connectedCallback() {
      console.log(`[BaseElement] Connected: ${this.elementName}`);
      if (this.initialized) return;
      
      // Initialize framework only once
      if (!elementInitialized) {
        try {
          console.log('[BaseElement] First element initialization - setting up framework');
          await elevateElementRoutes(BaseElement);
          elementInitialized = true;
          console.log('[BaseElement] Framework initialization complete');
        } catch (error) {
          console.error('[BaseElement] Initialization error:', error);
        }
      }
    ...
    attributeChangedCallback(name, oldValue, newValue) {
      if (name === 'config' && newValue) {
        try {
          ...
          this.dispatchEvent(new CustomEvent('config-updated', { detail: config }));
        } catch (error) {
